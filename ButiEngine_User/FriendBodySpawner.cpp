...
void ButiEngine::FriendBodySpawner::UpdateSpawnPattern()
{
	...
	case 8:
		if (m_spawnedBodiesNumber < 1)
		{
			SpawnBody(Vector3(-4.5, -4, 0), 0.05f, 0, 0);
	...
	case 10:
		if (m_spawnedBodiesNumber < 1)
		{
			SpawnBody(Vector3(4.5, -4, 0), -0.1f, -5, 0);
	...
	case 11:
		if (m_spawnedBodiesNumber < 1)
		{
			SpawnBody(Vector3(4.5, -4, 0), -0.2f, -100, 0);
	...
	case 12:
		if (m_spawnedBodiesNumber < 2)
		{
			SpawnBody(Vector3(-4.5f, -4, 0), 0.05f, 2, -180);
			SpawnBody(Vector3(4.5f, -4, 0), -0.05f, -2, 180);
	...
	case 13:
		if (m_vlp_spawnTimer->Update())
		{
			m_vlp_spawnTimer->ChangeCountFrame(ButiRandom::GetInt(30, 61));
			SpawnBody(Vector3(-4.5f, -4, 0), ButiRandom::GetRandom(0.05f, 0.2f), ButiRandom::GetRandom(1.0f, 4.0f), ButiRandom::GetRandom(0.0f, 360.0f));
		}
		break;
	}
